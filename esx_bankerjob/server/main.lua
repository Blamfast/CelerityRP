...
RegisterServerEvent('esx_bankerjob:LogDepotSociete')
AddEventHandler('esx_bankerjob:LogDepotSociete', function(amount, raison, deposant)
	local _source = source
	local sourceXPlayer = ESX.GetPlayerFromId(_source)
	local xDeposant = ESX.GetPlayerFromId(deposant)
	MySQL.Async.fetchAll('INSERT INTO depot_societe_bank (identifier_banquier, identifier_deposant, montant, raison) VALUES (@banquier, @deposeur, @montant, @raison)', {
		['@banquier'] = sourceXPlayer.identifier,
		['@deposeur'] = xDeposant.identifier,
		['@montant']  = amount,
		['@raison']   = raison
	}, function(result)
	end)
...
RegisterServerEvent('esx_bankerjob:RetraitSociete')
AddEventHandler('esx_bankerjob:RetraitSociete', function(retirant, societe, amount, raison)
	local xPlayer = ESX.GetPlayerFromId(source)
	local xTarget = ESX.GetPlayerFromId(retirant)
	TriggerEvent('esx_addonaccount:getSharedAccount', societe, function(account)
		if amount > 0 and account.money >= amount then
			account.removeMoney(amount)
			xTarget.addMoney(amount)	
			TriggerClientEvent('esx:showNotification', xPlayer.source, 'Un versement de ~g~'..amount..'$ a été effectué sur le compte du client')
			MySQL.Async.fetchAll('INSERT INTO retrait_societe_bank (identifier_banquier, identifier_retirant, montant, raison) VALUES (@banquier, @deposeur, @montant, @raison)', {
				['@banquier'] = xPlayer.identifier,
				['@deposeur'] = xTarget.identifier,
				['@montant']  = amount,
				['@raison']   = raison
			}, function(result)
			end)
		else
			TriggerClientEvent('esx:showNotification', xPlayer.source, 'Pas assez d\'argent sur le compte de la société')
		end
...
ESX.RegisterServerCallback('esx_bankerjob:GetArgentSociete', function(source, cb, societe)
	TriggerEvent('esx_addonaccount:getSharedAccount', societe, function(account)
		cb(account.money)
	end)
end)
  ...
  SetTimeout(3000, function()
    ...
    if PlayersSellingBil[source] == true and societyAccount ~= nil then
      ...
      if BQuantity == 0 then
        ...
        societyAccount.addMoney(Config.PriceResell.billet)
