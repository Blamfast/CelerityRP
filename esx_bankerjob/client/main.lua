...
local raisonDepot			  = nil
local raisonRetrait			  = nil
ESX                           = nil
...
function OpenBankActionsMenu()
	local elements = {
		...
		{ label = 'Menu Crédit', value = 'creditlecouz'}
	}

	if ESX.PlayerData.job.grade_name == 'boss' then
		table.insert(elements, { label = _U('boss_actions'), value = 'boss_actions' })
		table.insert(elements, { label = 'Gestion Sociétés', value = 'societelecouz' })
	end
    ...
    css    = 'entreprise',
		...
		if data.current.value == 'customers' then
			OpenCustomersMenu()
		elseif data.current.value == 'societelecouz' then
			MenuSociete()
		elseif data.current.value == 'creditlecouz' then
			ESX.ShowNotification("Ici on fout le menu crédit")
...
function MenuSociete()
	local elements = {
		{label = 'E.M.S', value = 'ambulance'},
		{label = 'Ammunation', value = 'ammunation'},
		{label = 'Benny\'s', value = 'bennys'},
		{label = 'Concessionnaire', value = 'cardealer'},
		{label = 'LS Mechanics', value = 'groove'},
		{label = 'McDonald', value = 'mcdonald'},
		{label = 'NightClub', value = 'nightclub'},
		{label = 'O\'Neil Works', value  = 'oneil'},
		{label = 'L.S.P.D', value = 'police'},
		{label = 'Agence Immobilière', value = 'realestateagent'},
		{label = 'Tabac', value = 'tabac'},
		{label = 'Unicorn', value = 'unicorn'},
		{label = 'Vignerons', value = 'vigne'}
	}
	
	ESX.UI.Menu.CloseAll()
	
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'societes', {
		title    = 'Comptes Sociétés',
		align    = 'top-right',
		css    = 'entreprise',
		elements = elements
	}, function(data, menu)
		local societe = data.current.value
		ModificationCompte(societe)
		menu.close()
	end)
end

function ModificationCompte(society)
	ESX.TriggerServerCallback('esx_bankerjob:GetArgentSociete', function(argent)
		if argent ~= nil then
			local elements = {
				{label = 'Montant Compte: '..argent..'$', value = ''},
				{label = 'Dépôt d\'argent', value = 'depot'},
				{label = 'Retrait d\'argent', value = 'retrait'}
			}
			ESX.UI.Menu.CloseAll()
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'argent_societe', {
				title = 'Argent de la société : '..society,
				align = 'top-right',
				css = 'entreprise',
				elements = elements
			}, function(data, menu)
				...
				if action == 'depot' then
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'depot_societe',
					{
					...
					function(data2, menu2)
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Log(amount, GetPlayerServerId(closestPlayer))
								
								TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_'..society, 'Dépôt Argent Société', amount)
							end
						end
				...
				elseif action == 'retrait' then
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'retrait_societe',
					{
					...
					function(data2, menu2)
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								local playerPed        = GetPlayerPed(-1)
								LogRetrait(amount, GetPlayerServerId(closestPlayer), society)
...
function LogRetrait(amount, idTarget, society)
	local raison = KeyboardInput("fakeid_VORNAMEN", 'Quelle est la raison du dépôt ?', "", 15)
	...
	TriggerServerEvent('esx_bankerjob:RetraitSociete', idTarget, 'society_'..society, amount, raisonRetrait)
	raisonRetrait = nil
end

function Log(amount, idTarget)
	local raison = KeyboardInput("fakeid_VORNAMEN", 'Quelle est la raison du dépôt ?', "", 15)
	...
	TriggerServerEvent('esx_bankerjob:LogDepotSociete', amount, raisonDepot, idTarget)
	raisonDepot = nil
end
...
function setUniform(job, playerPed)
  TriggerEvent('skinchanger:getSkin', function(skin)
    ...
    if skin.sex == 0 then
      if Config.Bank[job].male ~= nil then
        TriggerEvent('skinchanger:loadClothes', skin, Config.Bank[job].male)
      else
        ESX.ShowNotification(_U('no_outfit'))
      end
    else
      if Config.Bank[job].female ~= nil then
        TriggerEvent('skinchanger:loadClothes', skin, Config.Bank[job].female)
      else
        ESX.ShowNotification(_U('no_outfit'))
      end
    end
...
function OpenCloakroomMenu()
  ...
  local elements = {
    ...
    { label = _U('banker_outfit'),    value = 'banker_outfit'},
  }
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      if data.current.value == 'citizen_wear' then
		TriggerServerEvent("player:serviceOff", "banker")
        ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
          TriggerEvent('skinchanger:loadSkin', skin)
        end)
      end

      if data.current.value == 'banker_outfit' then
		TriggerServerEvent("player:serviceOn", "banker")
    ...
    ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(garageVehicles)
      ...
      ESX.UI.Menu.Open(
        ...
        function(data, menu)
          ...
          TriggerServerEvent('esx_society:removeVehicleFromGarage', 'banker', vehicleProps)
...
function OpenSocietyActionsMenu()
  ...
  if (ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'banker') then
  	table.insert(elements, {label = _U('gps'),    value = 'menu_gps'})
  end
  table.insert(elements, {label = "Annonces",    value = 'menu_announce'})

  ESX.UI.Menu.CloseAll()

  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'banker_actions',
    {
      css    = 'entreprise',
      title    = _U('banker'),
      align    = 'top-left',
    ...
    function(data, menu)
	  ...
	  if data.current.value == 'menu_announce' then
        OpenAnnounceMenu()
      end

	   if data.current.value == 'menu_gps' then
        ESX.UI.Menu.Open(
						'default', GetCurrentResourceName(), 'menu_gps',
						{
							css    = 'entreprise',
							title    = 'GPS Banque',
							align    = 'top-left',
							elements = {
								{label = 'Entreprise',     value = 'menu_gps_entreprise'},
								{label = 'Récolte Billet', value = 'menu_gps_billet'},
								{label = 'Vente Billet',   value = 'menu_gps_billet2'}
							},
						},
						function(data2, menu2)

							if data2.current.value == 'menu_gps_entreprise' then
								SetNewWaypoint( 252.652, 216.179, 106.118 )
								local source = GetPlayerServerId();
								ESX.ShowNotification("~b~Destination ajouté au GPS !")
							end

							if data2.current.value == 'menu_gps_billet' then
								SetNewWaypoint( -91.98, 6469.92, 30.46 ) 
								local source = GetPlayerServerId();
								ESX.ShowNotification("~b~Destination ajouté au GPS !")
							end

							if data2.current.value == 'menu_gps_billet2' then
								SetNewWaypoint( 277.91, 266.3, 104.67 )
								local source = GetPlayerServerId();
								ESX.ShowNotification("~b~Destination ajouté au GPS !")
							end
...
function OpenAnnounceMenu()
    ...
    local elements = {
      {label = _U('open_announce'), value = 'open_announce'},
      {label = _U('close_announce'), value = 'close_announce'}
    }
    ...
    ESX.UI.Menu.Open(
      'default', GetCurrentResourceName(), 'announce_menu',
      {
        title    = _U('announce_menu'),
        align    = 'top-left',
        css    = 'entreprise',
        elements = elements,
      },
      function(data, menu)
        ...
        if data.current.value == 'open_announce' then
            if messageNotFinished == true then
				TriggerServerEvent('AnnounceBanqueFerme')
            else
                messageNotFinished = true
				TriggerServerEvent('AnnounceBanqueOuverte')
            end
        end

        if data.current.value == 'close_announce' then
            if messageNotFinished == true then
				TriggerServerEvent('AnnounceBanqueFerme')
            else
                messageNotFinished = true
...
RegisterNetEvent('esx_bankerjob:announce')
AddEventHandler('esx_bankerjob:announce', function(text)
 	texteafiche = text
    affichenews = true
end)

RegisterNetEvent('esx_bankerjob:announcestop')
AddEventHandler('esx_bankerjob:announcestop', function()
    affichenews = false
    Wait(6)
 ...
 Citizen.CreateThread(function()
	while true do
		...
		if (affichenews == true) then
			...
			DrawAdvancedTextCNN(0.588, 0.14, 0.005, 0.0028, 0.8, "~p~~h~ Banque Centrale ~d~", 255, 255, 0, 255, 1, 0)
    ...
    function(data, menu)
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_banker', _U('billing'), amount)
        end
...
function OpenGetFridgeStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_bankerjob:getFridgeStockItems', function(items)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_bankerjob:getFridgeStockItem', itemName, count)
            end
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_bankerjob:putFridgeStockItems', itemName, count)
            end
...
AddEventHandler('esx_bankerjob:hasEnteredMarker', function(zone)
	...
	if zone == 'BFarm' then
            if (ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'banker') then
                CurrentAction     = 'bil_harvest'
                CurrentActionMsg  = _U('press_collect_bil')
                CurrentActionData = {}
            end
    end

    if zone == 'BResell' then
            if (ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'banker') then
                    CurrentAction     = 'bil_resell'
                    CurrentActionMsg  = _U('press_sell_bil')
                    CurrentActionData = {}
            end
    end
...
AddEventHandler('esx_bankerjob:hasExitedMarker', function(zone)
	...
	TriggerServerEvent('esx_bankerjob:stopHarvestBil')
    TriggerServerEvent('esx_bankerjob:stopSellBil')
  ...
  while true do
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlJustReleased(0,  Keys['E']) and (ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'banker') then
		...
		if CurrentAction == 'bil_harvest' then
            TriggerServerEvent('esx_bankerjob:startHarvestBil')
        end

		if CurrentAction == 'bil_resell' then
            TriggerServerEvent('esx_bankerjob:startSellBil')
        end
        ...
        if CurrentAction == 'delete_vehicle' then
          ...
          if Config.EnableSocietyOwnedVehicles then
            ...
            TriggerServerEvent('esx_society:putVehicleInGarage', 'banker', vehicleProps)
            ...
            then
              TriggerServerEvent('esx_service:disableService', 'banker')
            end
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsControlJustReleased(0,  Keys['F6']) and (ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'banker') and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'banker_actions') then
