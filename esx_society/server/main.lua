...
RegisterServerEvent('esx_society:clearGarage')
AddEventHandler('esx_society:clearGarage', function(societyName,societyveh)
	local society = GetSociety(societyName)
		...
		['@owner'] = societyveh
  		...
  		TriggerEvent('esx_datastore:getSharedDataStore', society.datastore, function(store)
			...
			for i=1, #result, 1 do
				...
				if true then
		  			print("Je supprime un véhicule de "..societyName)
...
RegisterServerEvent('esx_society:putVehiclesInGarage')
AddEventHandler('esx_society:putVehiclesInGarage', function(societyName, societyveh)
  local society = GetSociety(societyName)
  ...
  MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner', {
	['@owner'] = societyveh
		...
		print(#result.." vehicules trouvées pour la société "..societyName)
		for i=1, #result, 1 do
...
RegisterServerEvent('esx_society:putVehiclesInGaragePolice')
AddEventHandler('esx_society:putVehiclesInGaragePolice', function(societyveh)
  ...
  MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner', {
	['@owner'] = societyveh
		...
		for i=1, #result, 1 do
			local vehicle = json.decode(result[i].vehicle)
			TriggerEvent('esx_society:putVehicleInGarage', 'police', vehicle)
		end
	end)
end)
...
ESX.RegisterServerCallback('esx_society:getEmployees2', function(source, cb, society)
	if Config.EnableESXIdentity then
		...
		MySQL.Async.fetchAll('SELECT name, identifier, job2, job2_grade FROM users WHERE job2 = @job2 ORDER BY job2_grade DESC', {
			['@job2'] = society
		}, function (result)
			local employees = {}
...
function WashMoneyCRON(h, d, m)
	print("[BLANCHIMENT] : On blanchit l'argent")
...
TriggerEvent('cron:runAt', 3, 3, WashMoneyCRON)
